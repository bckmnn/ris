...
public final class QuaternionImp implements Quaternion {
   ...
   */
  public QuaternionImp(float x, float y, float z, float w) {
    float mag;
  ...
  public static Quaternion rotate(Vector axis, float angle) {
    return new QuaternionImp(MatrixImp.rotate(axis, angle));
  }
  ...
  public final Quaternion negate() {
    return new QuaternionImp(-x, -y, -z, -w);
  }
  ...
  public final Quaternion mul(Quaternion q) {
    return new QuaternionImp(w * q.x() + q.w() * x + y * q.z() - z * q.y(), w
        * q.y() + q.w() * y - x * q.z() + z * q.x(), w * q.z() + q.w() * z + x
        * q.y() - y * q.x(), w * q.w() - x * q.x() - y * q.y() - z * q.z());
  }
  ...
  public final Quaternion inverse() {
    ...
    return new QuaternionImp(norm * w, -norm * x, -norm * y, -norm * z);
  }
  ...
  public final Quaternion normalize() {
    ...
    if (norm > 0.0f) {
      ...
      return new QuaternionImp(x * norm, y * norm, z * norm, w * norm);
    } else {
      return new QuaternionImp();
    }
  }
  ...
  public final Quaternion interpolate(Quaternion q, float alpha) {
    ...
    return new QuaternionImp(s1 * w + s2 * q.w(), s1 * x + s2 * q.x(), s1 * y
        + s2 * q.y(), s1 * z + s2 * q.z());
  }
}